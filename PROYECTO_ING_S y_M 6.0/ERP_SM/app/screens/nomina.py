...
class NominaScreen(Screen):
    ...
    def generar_pdf(self):
        try:
            ...
            conn = get_connection()
            c = conn.cursor()
            c.execute("SELECT * FROM empleados WHERE id=?", (id_empleado,))
            emp = c.fetchone()
            conn.close()
            
            empleado = {
                "codigo": f"{emp[0]:05d}",
                "nombre": f"{emp[1]} {emp[2]}".upper(),
                "cedula": f"V-{emp[3]}",
                "cargo": emp[7].upper() if emp[7] else "",
                "departamento": emp[6].upper() if emp[6] else "N/A",
                "sueldo": emp[8],
                "fecha_ingreso": emp[9]
            }
            
            fecha = self.ids.btn_fecha.text
            periodo = self.ids.spinner_periodo.text.upper()
            
            ruta_pdf = generar_pdf_recibo(empleado, periodo, fecha, self.data_nomina)
